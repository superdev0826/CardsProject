...
import {
  getUserData,
  removeUserCardFav,
  updateUserCardsAddFav,
} from "../services/userSer"
...
function CardsList(props) {
  ...
  return (
    <div className="row">
      {props.ar.map((item) => {
        let bg =
          item.bizImage?.length > 2 ? item.bizImage : "/images/default.jpg"
        return (
          <div key={item._id} className="col-lg-4 p-3">
            <div className="border">
              ...
              <article className="p-3">
                ...
                <hr />
                <div>
                  <strong>Phone:</strong> {item.bizPhone}
                </div>
                <div>
                  <strong>Address:</strong> {item.bizAddress}
                </div>
                <div>
                  <strong>Biz number:</strong> {item.bizNumber}
                </div>
                {userData._id ? (
                  showBtnFav(item)
                ) : (
                  <small className="text text-danger">
                    * log in to add to favorite
                  </small>
                )}
              </article>
            </div>
